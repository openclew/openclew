...
| Prefix | Usage | Grep |
|--------|-------|------|
| `clw_ref@` | Refdoc (living knowledge) | `grep "clw_ref@"` |
...
|------|----------|--------|
| Refdoc | `doc/ref/SUBJECT.md` | UPPER_SNAKE_CASE, in `doc/ref/` |
...
**Line 1 prefix**: must start with `clw_ref@` (refdocs) or `clw_log@` (logs). Files starting with the legacy `openclew@` prefix are still parsed. Files starting with other prefixes (e.g. `R.AlphA.Doc@`) are not parsed by the openclew toolchain but remain valid Markdown.
