...
function collectFiles(docDir) {
  ...
  // Refdocs: doc/ref/*.md (new convention) + _*.md in docDir (legacy)
  if (fs.existsSync(docDir)) {
    for (const filepath of walkDir(docDir, REFDOC_EXTRA_SKIP).sort()) {
      const name = path.basename(filepath);
      if (SKIP_FILES.has(name) || !name.endsWith(".md")) continue;
      // New convention: any .md inside doc/ref/
      const relDir = path.relative(docDir, path.dirname(filepath));
      if (relDir === "ref" || relDir.startsWith("ref" + path.sep)) {
        files.push({ filepath, filename: name, kind: "refdoc" });
      } else if (name.startsWith("_")) {
        // Legacy convention: _*.md in doc/ root
        files.push({ filepath, filename: name, kind: "refdoc" });
      }
    }
  }
...
/**
 * Extract TODO section from an instruction file into TODO.md.
 *
 * Scans the entry point (AGENTS.md, CLAUDE.md, etc.) for a ## TODO section
 * containing checkboxes. Extracts them into TODO.md and replaces the section
 * with a pointer.
 */
function runTodo() {
  const args = process.argv.slice(2);
  ...
  // Find the instruction file (entry point)
  const { getEntryPoint } = require("./config");
  const { detectInstructionFiles } = require("./detect");

  let entryFile = getEntryPoint(projectRoot);
  if (!entryFile) {
    // Fallback: detect instruction files
    const detected = detectInstructionFiles(projectRoot).filter((f) => !f.isDir);
    if (detected.length > 0) {
      entryFile = detected[0].file;
    }
  }

  if (!entryFile) {
    console.error("No instruction file found. Run 'openclew init' first.");
    process.exit(1);
  }

  const entryPath = path.join(projectRoot, entryFile);
  if (!fs.existsSync(entryPath)) {
    console.error(`Entry point ${entryFile} not found.`);
    process.exit(1);
  }

  console.log(`openclew migrate --todo ${writeMode ? "(write mode)" : "(dry-run)"}\n`);
  console.log(`  Scanning ${entryFile} for TODO section...\n`);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (todoStart >= 0 && /^## /.test(trimmed) && !/^## TODO\b/i.test(trimmed)) {
      todoEnd = i;
      break;
    }
  }
  ...
  if (todoStart < 0) {
    console.log("  No ## TODO section found. Nothing to extract.");
    return;
  }

  // Extract TODO lines (checkboxes + subsection headers + blank lines)
  const todoLines = lines.slice(todoStart + 1, todoEnd);
  const checkboxes = todoLines.filter((l) => /^\s*- \[[ x]\] /i.test(l.trim()));

  if (checkboxes.length === 0) {
    console.log("  ## TODO section found but contains no checkboxes. Nothing to extract.");
    return;
  }

  console.log(`  Found ${checkboxes.length} task(s) in ${entryFile}\n`);
  ...
  const todoContent = todoLines
    .filter((l) => l.trim() !== "")
    .join("\n");
  const newTodoMd = `# TODO\n\n${todoContent}\n`;

  // Build replacement for the instruction file
  const pointer = `## TODO\n\nSee [\`TODO.md\`](TODO.md)\n`;
  const newLines = [
  ...
  if (writeMode) {
    // Write TODO.md (append if exists)
    if (fs.existsSync(todoPath)) {
      const existing = fs.readFileSync(todoPath, "utf-8");
      fs.writeFileSync(todoPath, existing.trimEnd() + "\n\n" + todoContent + "\n", "utf-8");
      console.log(`\n  ✓ Appended ${checkboxes.length} task(s) to TODO.md`);
    } else {
      fs.writeFileSync(todoPath, newTodoMd, "utf-8");
      console.log(`\n  ✓ Created TODO.md with ${checkboxes.length} task(s)`);
    }

    // Rewrite instruction file
    fs.writeFileSync(entryPath, newContent, "utf-8");
    console.log(`  ✓ Replaced ## TODO in ${entryFile} with pointer`);
  } else {
    console.log(`\n  Would create/update TODO.md with ${checkboxes.length} task(s)`);
    console.log(`  Would replace ## TODO in ${entryFile} with pointer`);
    console.log(`\nRun 'openclew migrate --todo --write' to apply.`);
  }
}

// ── Exports ─────────────────────────────────────────────────────────

module.exports = { parseLegacyDoc, migrateContent, collectFiles, repointRelatedDocs, runTodo, run };
