...
function refContent(title) {
  const date = today();
  const ver = ocVersion();
  return `clw_ref@${ver} · created: ${date} · updated: ${date} · doc_version: 1.0.0 · type: Reference · status: Active · category: · keywords: []
...
function guideContent() {
  const date = today();
  const ver = ocVersion();
  return `clw_ref@${ver} · created: ${date} · updated: ${date} · doc_version: 1.0.0 · type: Guide · status: Active · category: Documentation · keywords: [openclew, L1, L2, L3, index, ref, log]
...
`
  ...
  return `clw_ref@${ver} · created: ${date} · updated: ${date} · doc_version: 1.0.0 · type: Reference · status: Active · category: Architecture · keywords: [architecture, overview, components]
...
function frameworkIntegrationContent() {
  const date = today();
  const ver = ocVersion();
  return `clw_ref@${ver} · created: ${date} · updated: ${date} · doc_version: 1.0.0 · type: Guide · status: Active · category: Integration · keywords: [BMAD, Spec Kit, Kiro, workflow, framework, integration]
...
/**
 * Individual TODO doc — one file per task in doc/todo/.
 * Kept intentionally short. The point is to capture WHY it matters
 * and what done looks like — not implementation details.
 */
function todoDocContent(title) {
  const date = today();
  const ver = ocVersion();
  return `clw_todo@${ver} · created: ${date} · status: Open · priority: Normal · category: · keywords: []

- **subject:** ${title}
- **doc_brief:**
- **entry_log:** <!-- optional: doc/log/*.md or doc/ref/*.md that motivated this TODO -->
- **exit_log:** <!-- filled at closing: log/session reference where this TODO was resolved -->
...
## Why it matters
<!-- What's the pain or opportunity? 1-2 lines. -->

## Done looks like
<!-- How will you know it's done? 1-2 lines. -->

<!-- Keep it short. Implementation details belong in the log when work starts. -->
`;
}
