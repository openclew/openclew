...
function guideContent() {
  const date = today();
  const ver = ocVersion();
  return `clw_ref@${ver} · created: ${date} · updated: ${date} · type: Guide · status: Active · category: Documentation · keywords: [openclew, L1, L2, L3, index, refdoc, log]
...
6. After significant work: propose creating or updating a refdoc. Logs are only for end-of-session wrap-up — never create a log as a first action
...
- Logs are frozen (immutable), refdocs evolve over time
-->
...
**With openclew:** Lessons go into \`doc/ref/LESSONS_LEARNED.md\` — a living refdoc, updated after each retro. Future sprints consult it automatically via the index.
...
|--------------------|---------------|--------|------------|
| PRD | Refdoc | \`doc/ref/PRD_FEATURE.md\` | Updated as requirements evolve |
| Architecture decision | Refdoc | \`doc/ref/ARCHITECTURE.md\` | Updated as design evolves |
...
| Tech spec | Refdoc | \`doc/ref/SPEC_FEATURE.md\` | Updated until implemented |
