...
- **Refdocs** (\`doc/ref/*.md\` or \`doc/_*.md\`) — architecture, conventions, decisions (evolve over time)
...
- **No matching doc? Propose creating a refdoc** — suggest \`npx openclew add ref "Title"\` to capture key information before writing code. Logs are for end-of-session summaries only.
...
- \`npx openclew add ref "Title"\` — create a refdoc
...
2. **Pick** — identify which refdocs relate to the user's request
