...
function collectGitActivity() {
  ...
  // Refs: doc/ref/*.md (new) + doc/_*.md (legacy)
  let refs = [];
  const refDir = path.join(DOC_DIR, "ref");
  if (fs.existsSync(refDir)) {
    refs.push(...fs.readdirSync(refDir).filter((f) => f.endsWith(".md")));
  }
  if (fs.existsSync(DOC_DIR)) {
    refs.push(...fs.readdirSync(DOC_DIR).filter((f) => f.startsWith("_") && f !== "_INDEX.md" && f.endsWith(".md")));
  }

  return { date, commits, uncommitted, files, existingLogs, refs };
}
...
function displaySummary(activity) {
  const { date, commits, uncommitted, existingLogs, refs } = activity;
  ...
  // Refs reminder
  if (refs.length > 0) {
    console.log("  📚 Refs — check if any need updating:");
...
function generateSessionLog(activity, actions) {
  ...
  const content = `clw_log@${ver} · date: ${date} · type: ${logType} · status: Done · category: · keywords: ${keywordsStr}
