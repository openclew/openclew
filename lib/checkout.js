...
function collectGitActivity() {
  ...
  // Refdocs: doc/ref/*.md (new) + doc/_*.md (legacy)
  let refdocs = [];
  const refDir = path.join(DOC_DIR, "ref");
  if (fs.existsSync(refDir)) {
    refdocs.push(...fs.readdirSync(refDir).filter((f) => f.endsWith(".md")));
  }
  if (fs.existsSync(DOC_DIR)) {
    refdocs.push(...fs.readdirSync(DOC_DIR).filter((f) => f.startsWith("_") && f !== "_INDEX.md" && f.endsWith(".md")));
  }
