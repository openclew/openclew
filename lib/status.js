...
function run() {
  ...
  // ── TODOs Done without exit_log ──────────────────────────────
  const doneTodosNoExit = todos.filter(
    (d) =>
      (d.meta.status || "").toLowerCase() === "done" &&
      (!d.meta.exit_log || d.meta.exit_log === "")
  );
  if (doneTodosNoExit.length) {
    console.log(`Done TODOs missing exit_log (${doneTodosNoExit.length}):`);
    for (const d of doneTodosNoExit) {
      const relPath = path.relative(projectRoot, d.filepath);
      const subject = d.meta.subject || d.filename;
      console.log(`  - ${relPath}  (${subject})`);
    }
    console.log("  Add an exit_log: line in L1 (log path or session reference).\n");
  }
