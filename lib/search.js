 ...
 * L1 fields (subject, doc_brief) across all refs and logs.
...
function parseMetadataLine(content) {
  ...
  const prefixMatch = firstLine.match(/^(openclew|clw_ref|clw_log|clw_todo)@/);
  if (!prefixMatch) return meta;

  const parts = firstLine.split(" · ");
  for (const part of parts) {
    const trimmed = part.trim();
    const pfx = trimmed.match(/^(openclew|clw_ref|clw_log|clw_todo)@(.+)/);
    if (pfx) {
      meta.format = pfx[1];
      meta.version = pfx[2];
      continue;
...
function parseL1(content) {
  ...
  const entryMatch = block.match(/\*\*entry_log:\*\*\s*(.+)/);
  if (entryMatch) meta.entry_log = stripComment(entryMatch[1]);

  const exitMatch = block.match(/\*\*exit_log:\*\*\s*(.+)/);
  if (exitMatch) meta.exit_log = stripComment(exitMatch[1]);

  const targetsMatch = block.match(/\*\*targets_todos:\*\*\s*(.+)/);
  if (targetsMatch) meta.targets_todos = stripComment(targetsMatch[1]);
...
const REF_EXTRA_SKIP = new Set(["log", "notes", "verify_logs", "todo"]);
...
function collectDocs(docDir) {
  ...
  // Refs: doc/ref/*.md (new convention) + _*.md in docDir (legacy)
  // Excludes log/, notes/, verify_logs/ subdirs
  if (fs.existsSync(docDir)) {
    const refs = walkDir(docDir, REF_EXTRA_SKIP)
      .filter((f) => {
        const name = path.basename(f);
        if (!name.endsWith(".md") || SKIP_FILES.has(name)) return false;
        // New convention: any .md inside doc/ref/
        const relDir = path.relative(docDir, path.dirname(f));
        if (relDir === "ref" || relDir.startsWith("ref" + path.sep)) return true;
      ...
      .map((filepath) => {
        ...
        return meta ? { filepath, filename: path.basename(filepath), kind: "ref", meta } : null;
      })
      .filter(Boolean);
    docs.push(...refs);
  }
  ...
  // TODOs: *.md recursively under docDir/todo/
  const todoDir = path.join(docDir, "todo");
  if (fs.existsSync(todoDir)) {
    const todos = walkDir(todoDir)
      .filter((f) => path.basename(f).endsWith(".md"))
      .sort()
      .reverse()
      .map((filepath) => {
        const meta = parseFile(filepath);
        return meta ? { filepath, filename: path.basename(filepath), kind: "todo", meta } : null;
      })
      .filter(Boolean);
    docs.push(...todos);
  }
...
function run() {
  ...
  for (const r of results) {
    ...
    const icon = r.kind === "ref" ? "📘" : r.kind === "todo" ? "☐" : "📝";
