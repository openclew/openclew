...
function parseMetadataLine(content) {
  ...
  const prefixMatch = firstLine.match(/^(openclew|clw_ref|clw_log)@/);
  if (!prefixMatch) return meta;

  const parts = firstLine.split(" · ");
  for (const part of parts) {
    const trimmed = part.trim();
    const pfx = trimmed.match(/^(openclew|clw_ref|clw_log)@(.+)/);
    if (pfx) {
      meta.format = pfx[1];
      meta.version = pfx[2];
      continue;
...
const REF_EXTRA_SKIP = new Set(["log", "notes", "verify_logs"]);
