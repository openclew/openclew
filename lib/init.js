...
function installPromptFiles() {
  const promptsDir = path.join(PROJECT_ROOT, ".github", "prompts");
  ...
  for (const file of files) {
    ...
    const src = path.join(pkgPromptsDir, file);

    // Always overwrite — these are managed by openclew
    fs.copyFileSync(src, dest);
    installed++;
  }

  if (installed > 0) {
    console.log(`  Installed ${installed} prompt file(s) → .github/prompts/`);
    console.log("  Available in Copilot Chat: /oc-checkout, /oc-search, /oc-peek, /oc-status, /oc-init");
  } else {
    console.log("  No prompt files to install");
  }
}
...
async function main() {
  ...
  // Step 9: Install Copilot prompt files (.github/prompts/)
  console.log("\n9. Copilot prompts");
  installPromptFiles();
