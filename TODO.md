...
- [ ] **Init pollue `.gitignore` avec `doc/log/`** : `openclew init` ajoute `doc/log/` au `.gitignore` du projet cible, ce qui casse la convention R.AlphA (logs versionnés). Cas constaté : R.AlphA.Desktop commit 973178a (2026-04-08) — 5 logs perdus jusqu'au fix 2026-04-15. **Audit 2026-04-15** : seul R.AlphA.Desktop touché (20 autres projets R.AlphA.* clean). Hypothèse : bug introduit dans une version récente du template init. Actions : (1) retirer `doc/log/` du template `openclew init`, (2) vérifier la version du template qui a introduit la règle, (3) documenter la migration inverse (un-gitignore + rattrapage logs locaux)
